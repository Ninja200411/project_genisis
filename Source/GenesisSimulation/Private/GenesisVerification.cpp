...
namespace
{
FString StableGuid(const FGuid& Id)
{
    return Id.ToString(EGuidFormats::DigitsWithHyphensLower);
}
...
void AddDifference(
    ...
    FString Actual)
{
    FGenesisVerificationDifference Difference;
    Difference.Tick = Tick;
    Difference.EntityId = EntityId;
    Difference.Field = Field;
    Difference.Expected = MoveTemp(Expected);
    Difference.Actual = MoveTemp(Actual);
    Result.Add(MoveTemp(Difference));
}
}
...
FString FGenesisStateVerifier::Hash(const TArray<FGenesisCanonicalEntityState>& Entities)
{
    ...
    {
        return StableGuid(A.EntityId) < StableGuid(B.EntityId);
    });

    FString Canonical;
    for (const FGenesisCanonicalEntityState& Entity : Ordered)
    {
        Canonical += StableGuid(Entity.EntityId) + TEXT("|") + Entity.Type.ToString() + TEXT("|");
        TArray<FName> IntegerKeys;
        Entity.IntegerFields.GetKeys(IntegerKeys);
        IntegerKeys.Sort(FNameLexicalLess());
        for (const FName Key : IntegerKeys) Canonical += FString::Printf(TEXT("i:%s=%lld;"), *Key.ToString(), Entity.IntegerFields[Key]);
        TArray<FName> StringKeys;
        Entity.StringFields.GetKeys(StringKeys);
        StringKeys.Sort(FNameLexicalLess());
        for (const FName Key : StringKeys) Canonical += FString::Printf(TEXT("s:%s=%s;"), *Key.ToString(), *Entity.StringFields[Key]);
        Canonical += TEXT("\n");
    }
    ...
    const int64 Tick)
{
    ...
    for (const FGenesisCanonicalEntityState& ExpectedEntity : Expected)
    {
        ...
        {
            AddDifference(Result, Tick, ExpectedEntity.EntityId, TEXT("entity"), TEXT("present"), TEXT("missing"));
            continue;
        }
        for (const TPair<FName, int64>& Pair : ExpectedEntity.IntegerFields)
        {
            ...
            {
                AddDifference(Result, Tick, ExpectedEntity.EntityId, Pair.Key,
                    FString::Printf(TEXT("%lld"), Pair.Value), Value ? FString::Printf(TEXT("%lld"), *Value) : TEXT("missing"));
            }
        }
        for (const TPair<FName, FString>& Pair : ExpectedEntity.StringFields)
        {
            ...
            if (Value == nullptr || *Value != Pair.Value)
            {
                AddDifference(Result, Tick, ExpectedEntity.EntityId, Pair.Key, Pair.Value, Value ? *Value : TEXT("missing"));
            }
        }
    }
    Result.Sort([](const FGenesisVerificationDifference& A, const FGenesisVerificationDifference& B)
    {
        if (A.EntityId != B.EntityId) return StableGuid(A.EntityId) < StableGuid(B.EntityId);
